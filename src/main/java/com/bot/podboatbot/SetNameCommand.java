...
public final class SetNameCommand extends AnonymizerCommand {
    ...
    public void execute(AbsSender absSender, User user, Chat chat, String[] strings) {
        ...
        if (!mAnonymouses.hasAnonymous(user)) {
            log.log(Priority.INFO, "User {} is trying to execute '{}' without starting the bot!" + user.getId() + " " + getCommandIdentifier());
        ...
        if (displayedName == null) {
            log.log(Priority.INFO, "User {} is trying to set empty name." +
                    user.getId());
        ...
        if (mAnonymouses.setUserDisplayedName(user, displayedName)) {
            ...
            log.log(Priority.INFO, "User {} is trying to set taken name '{}'" + user.getId() + displayedName);
